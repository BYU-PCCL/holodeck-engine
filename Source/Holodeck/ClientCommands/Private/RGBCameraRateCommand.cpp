...
void URGBCameraRateCommand::Execute() {
	...
	verifyf(GameTarget != nullptr, TEXT("%s UCommand::Target is not a UHolodeckGameMode*."), TEXT(__FUNCTION__));
	...
	verifyf(ticksPerCapture > 0, TEXT("%s Invalid ticks per capture provided!"), TEXT(__FUNCTION__));

	AHolodeckAgent* Agent = GetAgent(AgentName);

	verifyf(Agent, TEXT("%s Could not find agent %s"), TEXT(__FUNCTION__), *AgentName);

	FString SensorName = StringParams[1].c_str();

	verifyf(Agent->SensorMap.Contains(SensorName), TEXT("%s Sensor %s not found on agent %s"), TEXT(__FUNCTION__), *SensorName, *AgentName);
